...
from localmask.engine import _scan_dir, _scan_file, _remask, _get_pro, _get_ner
...
class LocalMaskEngine:
        ...
        for det in scan["detections"]:
            if det["det_id"] in decisions:
                ...
                if decision in ("approved", "rejected"):
                    ...
                    updated += 1
                    if pro and pro.feedback(det, decision):
                        feedback_added += 1
    ...
    def get_model_stats(self) -> dict:
        pro = _get_pro()
        stats = pro.stats() if pro else None
        if not stats:
    ...
    def get_config(self) -> dict:
        ...
        pro = _get_pro()
        info = pro.info() if pro else {"ollama_available": False,
                                       "ollama_model": "none"}
        return {
            "ollama_available": info["ollama_available"],
            "ollama_model": info["ollama_model"],
